...
func CalculateSignature(headers HeaderOption, req *http.Request) string {
	...
	requ := restful.NewRequest(req)
	requ.SetAttribute(context.KContext, context.NewContext())
	calculatedSignature, err := Signer.Sign(requ, body, service, region, requestDateTime, requestDate, credentialStr)
	if err != nil {
...
func request(url string, method string, headers HeaderOption,
	...
	if "" == outFileName {
		...
		case "application/json":
			...
			break
		case "application/xml", "text/xml; charset=utf-8":
